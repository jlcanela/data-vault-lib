...
object UnitTest extends DefaultRunnableSpec {
  ...
  def spec = suite("Datavault unit tests")(
    ...
    testM("list files returns 9 files") {
      def show(file: ExtractLoad.File): ZIO[Console, Nothing, Unit] =
        console.putStrLn(file.toString)
    ...
    testM("table extract the headers") {
      ...
      assertM(headers.provideLayer(Datavault.deps))(
        equalTo(List("Year", "Make", "Model", "Description", "Price"))
      )
    },
    ...
    testM("tables extract rows") {
      ...
      def allHeaders = for {
        ...
        sink = Sink.foldLeft((List[List[String]]()))((list: List[List[String]], table: Table) =>
          table.headers.toList :: list
        )
        all <- tables.run(sink)
      } yield all
