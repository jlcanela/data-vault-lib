    ...
    ZLayer.succeed {
      new Service {
        ...
        def extractFiles(
          ...
          for {
            _           <- console.putStrLn("extract files")
            inputStream <- IO(Files.newInputStream(input))
            _           <- IO(Files.createDirectories(output))
            _           <- Archive.writeCsvFiles(inputStream, output)
          } yield ExtractFilesResult(true)
        ...
        def genModel(
          ...
          for {
            ...
            _           <- IO(Files.createDirectories(output.getParent()))
